...
class MatillionETL:
    ...
    def run_orchestration_job(self, job_name, environment='Production', variables=None):
        """
        Trigger an orchestration job in Matillion
        
        Parameters:
        -----------
        job_name : str
            Name of the orchestration job
        environment : str
            Environment to run in (Dev, Test, Production)
        variables : dict
            Job variables to pass
            
        Returns:
        --------
        dict: Job execution details
        ...
        """
        try:
            url = f"{self.base_url}/rest/v1/group/name/{self.config['group']}/project/name/{self.config['project']}/version/name/{self.config['version']}/environment/name/{environment}/job/name/{job_name}/run"
    ...
    def export_job_config(self, job_name, output_path, environment='Production'):
        """
        Export job configuration as JSON
        
        Parameters:
        -----------
        job_name : str
            Name of job to export
        output_path : str
